...
本仓库在原版基础上新增了**插件系统**，可以在不修改源码的前提下扩展、覆盖客户端的既有功能。管理入口：**设置 → 插件管理**。

支持的插件操作：

- **安装**：点击「上传安装插件」选择 `.lxplugin` 文件，或直接把文件**拖入**页面
- **更新**：上传同一插件 id 的新版本覆盖安装（新版本号需高于已安装版本）
- **启用 / 禁用**：插件安装后**默认启用**；禁用时不加载插件代码，原功能不受影响
- **卸载**：移除插件及其数据，被插件改动过的功能会**精确还原**为原实现

> 页面另提供「安装插件目录」「目录更新」入口，用于本地插件开发（直接指向插件源码目录）。

插件构建产物是**单个文件**（`.lxplugin`），内含插件清单与入口代码。安装阶段只解析清单与文本，**不执行插件代码**，因此代码有问题的插件不会导致安装失败。

插件可以做到：

- **覆盖 / 修改客户端原有功能**：通过运行时函数包装（`api.patch`）接管或增强既有函数，禁用、卸载时自动还原
- **监听生命周期事件、拦截并改写数据流**（`api.hooks`）
- **注册自定义音乐源**（`api.registerMusicSource`），参与跨源搜索
- **接管网络请求使用的代理 agent**（如让「设置 → 网络 → 网络代理」支持 SOCKS5）

仓库内置了 3 个实用插件（源码与构建脚本见 [`lx-plugins/`](./lx-plugins)，产物可直接上传安装）：

| 插件 | 作用 |
| --- | --- |
| `repo-source-plugins` | 音乐跨源搜索去重与统计增强，并演示如何注册自定义音乐源 |
| `sock_proxy` | 让「设置 → 网络 → 网络代理」支持 **SOCKS5** 代理（原实现仅支持 HTTP 代理） |
| `forbidden_update` | 安装启用后**禁用检查更新**，不再发起更新检查，也不再接收更新事件 |

插件系统对上游源码只有 5 处带 `=== Plugin Manager ===` 标记的极小改动，便于跟随上游更新合并；插件运行时位于 [`src/plugins`](./src/plugins)，随主程序打包。插件开发（目录结构、构建方式、API 一览、状态说明）详见 [`lx-plugins/README.md`](./lx-plugins/README.md)。
...
安装的插件存放在数据存储目录下的 `plugins` 子目录中，即 `<数据存储目录>/plugins/<插件 id>/`。
